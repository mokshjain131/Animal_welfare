...
```python
TOPIC_KEYWORDS = {
    "factory_farming":   ["factory farm", "battery cage", "gestation crate", "live export", "livestock welfare", ...],  # 16
    "animal_testing":    ["animal testing", "vivisection", "lab animals", ...],  # 9
    "wildlife":          ["wildlife trafficking", "wildlife trade", "wildlife conservation", "poaching", ...],  # 17
    "pet_welfare":       ["animal cruelty", "animal abuse", "dog fighting", "animal rescue", "animal shelter", ...],  # 15
    "animal_policy":     ["animal welfare", "animal rights", "animal protection", "animal welfare law", ...],  # 11
    "veganism":          ["vegan", "plant-based", "meat industry", ...],  # 10
}
```

> **Note:** Bare generic words like `"wildlife"`, `"biodiversity"`, `"habitat destruction"`, and `"deforestation"` were removed. All keywords are now multi-word compound phrases or unambiguous welfare terms to avoid false positives (e.g. mining reports, geography articles).
...
| **Relevance Gate** | Weighted keyword scoring — title match = +2, body match = +1, threshold = 2. Logs rejected articles |
...
| **Sentiment Analyser** | HuggingFace Inference API (`facebook/bart-large-mnli` zero-shot) | Animal-welfare-aware directional score 0–1 + label |
| **Topic Classifier** | HuggingFace Inference API (`facebook/bart-large-mnli` zero-shot) | Topic category + confidence score |
| **Misinformation Detector** | HuggingFace Inference API (`mrm8488/bert-tiny-finetuned-fake-news-detection`) | Suspicion score 0–1, flag if above threshold |
| **YAKE Extractor** | YAKE (local, no model download) | Top 3–5 statistical keyphrases with relevance scores |
...
| **TF-IDF Keyword Computer** | Every 30 min | Compares last 3 days vs 7-day baseline, filters for animal-welfare relevance, writes to `trending_keywords` |
...
| `yake` | Lightweight statistical keyphrase extraction (no GPU, no model download) |
| `requests` | Used by `hf_api.py` to call HuggingFace Inference API |

> No local `torch`, `transformers`, `keybert`, or `sentence-transformers` required. All heavy NLP inference runs via the HuggingFace Inference API through `nlp/hf_api.py`.

**HuggingFace models (via Inference API — `HF_API_TOKEN` required):**

| Task | Model | Notes |
|---|---|---|
| Sentiment | `facebook/bart-large-mnli` | Zero-shot with animal-welfare candidate labels; directional 0–1 score |
| Topic (zero-shot) | `facebook/bart-large-mnli` | Same model, different candidate labels |
| Misinformation | `mrm8488/bert-tiny-finetuned-fake-news-detection` | Text classification via API |
